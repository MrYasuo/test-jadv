...
const getCourse = async (username, password, type) => {
    const cookieJar = new CookieJar(`src/assets/cookies/${username}.json`);
    const urlType = type === "major" ? 1 : 2;
    // @ts-ignore
    await cookieJar.load();
    try {
        const res = await fetch(cookieJar, `http://dangkyhoc.vnu.edu.vn/danh-sach-mon-hoc/1/${urlType}`, {
            method: "POST",
            headers: {
        ...
        html = html
            .replace(/>\s+|\s+<|\s+\(|\)\s+/g, (m) => m.trim())
            .replace(/(<span>|<\/span>|<b>|<\/b>)/g, "")
            .replace(/(.)(\(TH\)|\(LT\))/g, "$1 $2");
        html = `<strong>Updated at ${luxon.DateTime.now().toFormat("dd/MM/yyyy HH:mm")}</strong><table>${html}</table>`;
