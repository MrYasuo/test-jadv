...
const loginVnu = async (username, password) => {
    ...
    const __RequestVerificationToken = fs.readFileSync(`src/assets/tokens/${username}.txt`, "utf8");
    const response = await fetch(cookieJar, "http://dangkyhoc.vnu.edu.vn/dang-nhap", {
        method: "POST",
        headers: {
    ...
    if (html.includes("<title>Trang chủ")) {
        ...
        throw new ApiError(403, "Please login with username and password of dangkyhoc.vnu.edu.vn");
    }
    else
        throw new ApiError(500, "Some error occured. Please try again later.");
};
